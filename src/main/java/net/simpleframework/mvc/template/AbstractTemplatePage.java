...
public abstract class AbstractTemplatePage extends AbstractBasePage {
	...
	public static UrlForward PAGE_404 = new UrlForward(url(Err404Page.class)).setRedirect(true);
	public static UrlForward PAGE_PERMISSION = new UrlForward(url(ErrPermissionPage.class))
			.setRedirect(true);
	...
	public static enum EPageRedirect {
		...
		public abstract UrlForward getDefaultPage();
	}

	protected IForward getMobilePageForward(final PageParameter pp) {
	...
	public IForward forward(final PageParameter pp) throws Exception {
		IForward forward = getMobilePageForward(pp);
		if (forward != null) {
			return forward;
		}
	...
	protected String js_imageZoom(final PageParameter pp) {
		return js_imageZoom(pp, true);
	}

	protected String js_imageZoom(final PageParameter pp, final boolean wrapScriptTag) {
		return js_imageZoom(pp, "img[data-zoomable]", wrapScriptTag);
	}
	...
	protected String js_imageZoom(final PageParameter pp, final String selector,
		...
		return wrapScriptTag ? JavascriptUtils.wrapScriptTag(sb.toString(), true)
				: JavascriptUtils.wrapWhenReady(sb.toString());
	}
